...
namespace static_meshes_3D {
	...
	void Cylinder::initializeData()
	{
		...
		if (hasPositions())
		{
			...
			for (auto i = 0; i <= _numSlices; i++)
			{
				const auto topPosition = glm::vec3(x[i], _height / 1.0f, z[i]);
				const auto bottomPosition = glm::vec3(x[i], -_height / 1.0f, z[i]);
				_vbo.addRawData(&topPosition, sizeof(glm::vec3));
				_vbo.addRawData(&bottomPosition, sizeof(glm::vec3));
			}

			// Add top cylinder cover
			glm::vec3 topCenterPosition(0.0f, _height / 1.0f, 0.0f);
			...
			glm::vec3 bottomCenterPosition(0.0f, -_height / 1.0f, 0.0f);
